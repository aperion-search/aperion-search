...
from aperion import settings
from aperion.data import USER_AGENTS, data_dir
from aperion.version import VERSION_TAG
from aperion.sxng_locales import sxng_locales
from aperion.exceptions import AperionXPathSyntaxException, AperionEngineXPathException
...
SEARCH_LANGUAGE_CODES = frozenset([aperion_locale[0].split('-')[0] for aperion_locale in sxng_locales])
"""Languages supported by most aperion engines (:py:obj:`aperion.sxng_locales.sxng_locales`)."""
...
def aperion_useragent() -> str:
    """Return the aperion User Agent"""
    return f"aperion/{VERSION_TAG} {settings['outgoing']['useragent_suffix']}".strip()


def gen_useragent(os_string: Optional[str] = None) -> str:
    """Return a random browser User Agent

    See aperion/data/useragents.json
    """
...
def extract_url(xpath_results, base_url) -> str:
    ...
    Example:
        >>> def f(s, search_url):
        >>>    return aperion.utils.extract_url(html.fromstring(s), search_url)
        ...
        >>> aperion.utils.extract_url([], 'https://example.com')
        raise ValueError
...
def is_valid_lang(lang) -> Optional[Tuple[bool, str, str]]:
    ...
    Examples:
        ...
        >>> is_valid_lang('en')
        (True, 'en', 'english')
        >>> aperion.utils.is_valid_lang('Español')
        (True, 'es', 'spanish')
        >>> aperion.utils.is_valid_lang('Spanish')
        (True, 'es', 'spanish')
    """
...
def get_xpath(xpath_spec: XPathSpecType) -> XPath:
    ...
    Raises:
        ...
        * AperionXPathSyntaxException: Raise when there is a syntax error in the XPath
    """
    if isinstance(xpath_spec, str):
        ...
        if result is None:
            try:
                ...
                raise AperionXPathSyntaxException(xpath_spec, str(e.msg)) from e
...
def eval_xpath(element: ElementBase, xpath_spec: XPathSpecType):
    ...
    Raises:
        * TypeError: Raise when xpath_spec is neither a str nor a lxml.etree.XPath
        * AperionXPathSyntaxException: Raise when there is a syntax error in the XPath
        * AperionEngineXPathException: Raise when the XPath can't be evaluated.
    """
    ...
    try:
        ...
        raise AperionEngineXPathException(xpath_spec, arg) from e
...
def eval_xpath_list(element: ElementBase, xpath_spec: XPathSpecType, min_len: Optional[int] = None):
    ...
    Raises:
        * TypeError: Raise when xpath_spec is neither a str nor a lxml.etree.XPath
        * AperionXPathSyntaxException: Raise when there is a syntax error in the XPath
        * AperionEngineXPathException: raise if the result is not a list
    ...
    if not isinstance(result, list):
        raise AperionEngineXPathException(xpath_spec, 'the result is not a list')
    if min_len is not None and min_len > len(result):
        raise AperionEngineXPathException(xpath_spec, 'len(xpath_str) < ' + str(min_len))
    return result
...
def eval_xpath_getindex(elements: ElementBase, xpath_spec: XPathSpecType, index: int, default=_NOTSET):
    ...
    Raises:
        * TypeError: Raise when xpath_spec is neither a str nor a lxml.etree.XPath
        * AperionXPathSyntaxException: Raise when there is a syntax error in the XPath
        * AperionEngineXPathException: if the index is not found. Also see eval_xpath.
    ...
    if -len(result) <= index < len(result):
        return result[index]
    if default == _NOTSET:
        # raise an AperionEngineXPathException instead of IndexError
        # to record xpath_spec
        raise AperionEngineXPathException(xpath_spec, 'index ' + str(index) + ' not found')
    return default
...
def detect_language(text: str, threshold: float = 0.3, only_search_languages: bool = False) -> Optional[str]:
    ...
    :param bool only_search_languages: If ``True``, returns only supported
        aperion search languages.  see :py:obj:`aperion.languages`
    ...
    is harmonized with the aperion's language (locale) model.  General
    conditions of aperion's locale model are:

    a. aperion's locale of a query is passed to the
       :py:obj:`aperion.locales.get_engine_locale` to get a language and/or region
       code that is used by an engine.

    b. Most of aperion's engines do not support all the languages from `language
       identification model`_ and there is also a discrepancy in the ISO-639-3
       (fasttext) and ISO-639-2 (aperion)handling.  Further more, in aperion the
       locales like ``zh-TH`` (``zh-CN``) are mapped to ``zh_Hant``
       (``zh_Hans``) while the `language identification model`_ reduce both to
       ``zh``.

    .. _a fork: https://github.com/aperion/fasttext-predict
