...
that aperion passes through requests from bots and is thus classified as a bot
itself.  As a result, the aperion engine then receives a CAPTCHA or is blocked
by the search engine (the origin) in some other way.

To avoid blocking, the requests from bots to aperion must also be blocked, this
...
.. _NGINX RequestHeader:
    https://docs.aperion.org/admin/installation-nginx.html#nginx-s-aperion-site
.. _Apache RequestHeader:
    https://docs.aperion.org/admin/installation-apache.html#apache-s-aperion-site
...
file ``/etc/aperion/limiter.toml``.  The defaults are shown in limiter.toml_ /
...
import aperion.compat
from aperion import (
    logger,
    valkeydb,
)
from aperion import botdetection
from aperion.extended_types import SXNG_Request, sxng_request
from aperion.botdetection import (
    config,
    http_accept,
...
def get_cfg() -> config.Config:
    """Returns aperion's global limiter configuration."""
    global CFG  # pylint: disable=global-statement

    if CFG is None:
        from . import settings_loader  # pylint: disable=import-outside-toplevel

        cfg_file = (settings_loader.get_user_cfg_folder() or Path("/etc/aperion")) / "limiter.toml"
        CFG = config.Config.from_toml(LIMITER_CFG_SCHEMA, cfg_file, aperion.compat.LIMITER_CFG_DEPRECATED)
        aperion.compat.limiter_fix_cfg(CFG, cfg_file)
...
def initialize(app: flask.Flask, settings):
    ...
    if not valkey_client:
        logger.error(
            ...
            "https://docs.aperion.org/admin/aperion.limiter.html"
        )
