...
from aperion.webutils import VALID_LANGUAGE_CODE
from aperion.query import RawTextQuery
from aperion.engines import categories, engines
from aperion.search import SearchQuery, EngineRef
from aperion.preferences import Preferences, is_locked
from aperion.utils import detect_language
...
def parse_pageno(form: Dict[str, str]) -> int:
    ...
    if not pageno_param.isdigit() or int(pageno_param) < 1:
        raise AperionParameterException('pageno', pageno_param)
...
def parse_lang(preferences: Preferences, form: Dict[str, str], raw_text_query: RawTextQuery) -> str:
    ...
    if not VALID_LANGUAGE_CODE.match(query_lang) and query_lang != 'auto':
        raise AperionParameterException('language', query_lang)
...
def parse_safesearch(preferences: Preferences, form: Dict[str, str]) -> int:
    ...
    if 'safesearch' in form:
        ...
        if not query_safesearch.isdigit():
            raise AperionParameterException('safesearch', query_safesearch)
...
def parse_time_range(form: Dict[str, str]) -> Optional[str]:
    ...
    if query_time_range not in (None, 'day', 'week', 'month', 'year'):
        raise AperionParameterException('time_range', query_time_range)
...
def parse_timeout(form: Dict[str, str], raw_text_query: RawTextQuery) -> Optional[float]:
    ...
    try:
        ...
        raise AperionParameterException('timeout_limit', timeout_limit) from e
...
def get_search_query_from_webapp(
    ...
    1. instance of :py:obj:`aperion.search.SearchQuery`
    2. instance of :py:obj:`aperion.query.RawTextQuery`
    3. list of :py:obj:`aperion.search.EngineRef` instances
    ...
    <aperion.utils.detect_language>`.  If language recognition does not have a
    ...
    if not form.get('q'):
        raise AperionParameterException('q', '')
