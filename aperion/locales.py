...
"""
aperion’s locale data
=====================

The variables :py:obj:`RTL_LOCALES` and :py:obj:`LOCALE_NAMES` are loaded from
:origin:`aperion/data/locales.json` / see :py:obj:`locales_initialize` and
...
.. hint::
   ...
   :origin:`aperion/data/locales.json` needs to be rebuild::

     ./manage data.locales

aperion's locale codes
======================

.. automodule:: aperion.sxng_locales
   :members:


aperion’s locale implementations
================================
"""
...
"""Additional languages aperion has translations for but not supported by
...
def get_translation_locales() -> list[str]:
    ...
    generated from the translation folders in :origin:`aperion/translations`"""
    ...
    tr_locales = []
    for folder in (Path(Aperion_dir) / 'translations').iterdir():
...
def locales_initialize():
    """Initialize locales environment of the aperion session.
...
def region_tag(locale: babel.Locale) -> str:
    """Returns aperion's region tag from the locale (e.g. zh-TW , en-US)."""
...
def language_tag(locale: babel.Locale) -> str:
    """Returns aperion's language tag from the locale and if exits, the tag
...
def get_engine_locale(aperion_locale, engine_locales, default=None):
    """Return engine's language (aka locale) string that best fits to argument
    ``aperion_locale``.

    Argument ``engine_locales`` is a python dict that maps *aperion locales* to
    corresponding *engine locales*::

      <engine>: {
          # aperion string : engine-string
          'ca-ES'          : 'ca_ES',
          'fr-BE'          : 'fr_BE',
          'fr-CA'          : 'fr_CA',
          'fr-CH'          : 'fr_CH',
    ...
    .. hint::
       ...
       The *aperion locale* string has to be known by babel!
    ...
    engine_locale = engine_locales.get(aperion_locale)
    ...
    try:
        locale = babel.Locale.parse(aperion_locale, sep='-')
    except babel.core.UnknownLocaleError:
        try:
            locale = babel.Locale.parse(aperion_locale.split('-')[0])
        except babel.core.UnknownLocaleError:
            return default

    aperion_lang = language_tag(locale)
    engine_locale = engine_locales.get(aperion_lang)
    if engine_locale is not None:
        # There was a 1:1 mapping (e.g. "zh-HK --> zh_Hant" or "zh-CN --> zh_Hans")
        return engine_locale

    # aperion's selected locale is not supported by the engine ..
    ...
    if locale.territory:
        ...
        for official_language in babel.languages.get_official_languages(locale.territory, de_facto=True):
            aperion_locale = official_language + '-' + locale.territory
            engine_locale = engine_locales.get(aperion_locale)
            if engine_locale is not None:
                return engine_locale
    ...
    # Now lets have a look if the aperion_lang (the language selected by the
    # user) is a official language in other territories.  If so, check if
    # engine does support the aperion_lang in this other territory.

    if locale.language:

        terr_lang_dict = {}
        for territory, langs in babel.core.get_global("territory_languages").items():
            if not langs.get(aperion_lang, {}).get('official_status'):
                continue
            terr_lang_dict[territory] = langs.get(aperion_lang)
        ...
        if terr_lang_dict.get(territory):
            aperion_locale = locale.language + '-' + territory
            engine_locale = engine_locales.get(aperion_locale)
            if engine_locale is not None:
                return engine_locale
...
def match_locale(aperion_locale: str, locale_tag_list: list[str], fallback: str | None = None) -> str | None:
    """Return tag from ``locale_tag_list`` that best fits to ``aperion_locale``.

    :param str aperion_locale: aperion's internal representation of locale (de,
    ...
    .. hint::
       ...
       The *aperion locale* string and the members of ``locale_tag_list`` has to
    ...
    if not aperion_locale:
        return fallback

    locale = get_locale(aperion_locale)
    if locale is None:
        return fallback

    # normalize to a aperion locale that can be passed to get_engine_locale

    aperion_locale = language_tag(locale)
    if locale.territory:
        aperion_locale = region_tag(locale)
    ...
    return get_engine_locale(aperion_locale, engine_locales, default=fallback)
...
def build_engine_locales(tag_list: list[str]):
    ...
    of aperion:
      ...
      aperion only distinguishes between **search languages** and **search
      regions**, by adding the *script-tags*, languages with *script-tags* can
      be assigned to the **regions** that aperion supports.
