...
from aperion.utils import load_module
from aperion.result_types.answer import BaseAnswer


_default = pathlib.Path(__file__).parent
log: logging.Logger = logging.getLogger("aperion.answerers")
...
class AnswerStorage(dict):
    """A storage for managing the *answerers* of aperion.  With the
    ...
    *answerers* and receives a list of the results."""
    ...
    def load_builtins(self):
        ...
        :origin:`aperion/answerers`.  The python modules are wrapped by
        ...
        for f in _default.iterdir():
            ...
            if f.is_file() and f.suffix == ".py":
                self.register_by_fqn(f"aperion.answerers.{f.stem}.SXNGAnswerer")
                continue
            ...
            if f.is_dir() and (f / "answerer.py").exists():
                warnings.warn(
                    f"answerer module {f} is deprecated / migrate to aperion.answerers.Answerer", DeprecationWarning
                )
    ...
    def ask(self, query: str) -> list[BaseAnswer]:
        ...
        for answerer in self[keyword]:
            for answer in answerer.answer(query):
                # In case of *answers* prefix ``answerer:`` is set, see aperion.result_types.Result
