...
their mapping from aperion's representation to the representation in the origin
...
from aperion import locales
from aperion.data import data_dir, ENGINE_TRAITS
...
class EngineTraits:
    ...
    """Maps aperion's internal representation of a region to the one of the engine.

    aperion's internal representation can be parsed by babel and the value is
    send to the engine:
    ...
    .. code:: python
       ...
       for key, egnine_region regions.items():
          aperion_region = babel.Locale.parse(key, sep='-')
          ...
    """

    languages: Dict[str, str] = dataclasses.field(default_factory=dict)
    """Maps aperion's internal representation of a language to the one of the engine.
    ...
    .. code:: python
       ...
       for key, egnine_lang in languages.items():
          aperion_lang = babel.Locale.parse(key)
          ...
    """

    all_locale: Optional[str] = None
    """To which locale value aperion's ``all`` language is mapped (shown a "Default
    language").
    """
    ...
    """A place to store engine's custom traits, not related to the aperion core.
    """

    def get_language(self, aperion_locale: str, default=None):
        """Return engine's language string that *best fits* to aperion's locale.

        :param aperion_locale: aperion's internal representation of locale
          selected by the user.

        :param default: engine's default language

        The *best fits* rules are implemented in
        :py:obj:`aperion.locales.get_engine_locale`.  Except for the special value ``all``
        which is determined from :py:obj:`EngineTraits.all_locale`.
        """
        if aperion_locale == 'all' and self.all_locale is not None:
            return self.all_locale
        return locales.get_engine_locale(aperion_locale, self.languages, default=default)

    def get_region(self, aperion_locale: str, default=None):
        """Return engine's region string that best fits to aperion's locale.
        ...
        if aperion_locale == 'all' and self.all_locale is not None:
            return self.all_locale
        return locales.get_engine_locale(aperion_locale, self.regions, default=default)

    def is_locale_supported(self, aperion_locale: str) -> bool:
        """A *locale* (aperion's internal representation) is considered to be
        ...
        if self.data_type == 'traits_v1':
            return bool(self.get_region(aperion_locale) or self.get_language(aperion_locale))
    ...
    def set_traits(self, engine: Engine):
        ...
        :param engine: engine instance build by :py:func:`aperion.engines.load_engine`
        """
...
class EngineTraitsMap(Dict[str, EngineTraits]):
    ...
    def fetch_traits(cls, log: Callable) -> 'EngineTraitsMap':
        from aperion import engines  # pylint: disable=cyclic-import, import-outside-toplevel
        ...
        for engine_name in names:
            ...
            if traits is not None:
                log("%-20s: aperion languages --> %s " % (engine_name, len(traits.languages)))
                log("%-20s: aperion regions   --> %s" % (engine_name, len(traits.regions)))
