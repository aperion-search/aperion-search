...
"""Implementations of the framework for the aperion engines.

- :py:obj:`aperion.enginelib.EngineCache`
- :py:obj:`aperion.enginelib.Engine`
- :py:obj:`aperion.enginelib.traits`
...
an example in which the command line is called in the development environment::
  ...
  (py3) python -m aperion.enginelib --help
...
.. hint::
   ...
   - move implementations of the :ref:`aperion.engines loader` to a new module in
     the :py:obj:`aperion.enginelib` namespace.
...
if TYPE_CHECKING:
    from aperion.enginelib import traits
...
"""Global :py:obj:`aperion.cache.ExpireCacheSQLite` instance where the cached
...
class EngineCache:
    ...
    <aperion.cache.ExpireCacheCfg.MAXHOLD_TIME>`).  This class is a wrapper around
    :py:obj:`ENGINES_CACHE` (:py:obj:`ExpireCacheSQLite
    <aperion.cache.ExpireCacheSQLite>`).

    In the :origin:`aperion/engines/demo_offline.py` engine you can find an
    exemplary implementation of such a cache other examples are implemented
    in:

    - :origin:`aperion/engines/radio_browser.py`
    - :origin:`aperion/engines/soundcloud.py`
    - :origin:`aperion/engines/startpage.py`
    ...
    show cache state::
        ...
        $ ./manage pyenv.cmd bash --norc --noprofile
        (py3) python -m aperion.enginelib cache state
...
class Engine:  # pylint: disable=too-few-public-methods
    ...
    engine_type: str
    """Type of the engine (:ref:`aperion.search.processors`)"""
    ...
    name: str
    """Name that will be used across aperion to define this engine.  In settings, on
    the result page .."""

    engine: str
    """Name of the python file used to handle requests and responses to and from
    this search engine (file name from :origin:`aperion/engines` without
    ``.py``)."""
