...
from aperion import logger as log
import aperion.engines
from aperion.metrics import histogram_observe, counter_add
from aperion.result_types import Result, LegacyResult, MainResult
from aperion.result_types.answer import AnswerSet, BaseAnswer
...
def calculate_score(result, priority) -> float:
    ...
    for result_engine in result['engines']:
        if hasattr(aperion.engines.engines.get(result_engine), 'weight'):
            weight *= float(aperion.engines.engines[result_engine].weight)
...
class ResultContainer:
    ...
    def extend(self, engine_name: str | None, results):  # pylint: disable=too-many-branches
        ...
        if engine_name in aperion.engines.engines:
            eng = aperion.engines.engines[engine_name]
    ...
    def get_ordered_results(self) -> list[MainResult | LegacyResult]:
        ...
        for res in results:
            ...
            engine = aperion.engines.engines.get(res.engine or "")
            if engine:
    ...
    def add_unresponsive_engine(self, engine_name: str, error_type: str, suspended: bool = False):
        with self._lock:
            if self._closed:
                ...
                return
            if aperion.engines.engines[engine_name].display_error_messages:
...
def merge_two_infoboxes(origin: LegacyResult, other: LegacyResult):
    ...
    weight1 = getattr(aperion.engines.engines[origin.engine], "weight", 1)
    weight2 = getattr(aperion.engines.engines[other.engine], "weight", 1)
