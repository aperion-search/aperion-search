...
import aperion.unixthreadname
import aperion.settings_loader
from aperion.settings_defaults import SCHEMA, apply_schema
...
Aperion_parent_dir = abspath(dirname(dirname(__file__)))
...
def init_settings():
    ...
    ``logger`` from ``aperion_SETTINGS_PATH``.
    """

    global settings, sxng_debug  # pylint: disable=global-variable-not-assigned

    cfg, msg = aperion.settings_loader.load_settings(load_user_settings=True)
    cfg = cfg or {}
    ...
    if settings['server']['public_instance']:
        logger.warning(
            ...
            "see https://docs.aperion.org/admin/aperion.limiter.html"
        )
...
def _logging_config_debug():
    ...
    log_level = os.environ.get('aperion_DEBUG_LOG_LEVEL', 'DEBUG')
