...
from aperion.exceptions import (
    AperionXPathSyntaxException,
    AperionEngineXPathException,
    AperionEngineAPIException,
    AperionEngineAccessDeniedException,
)
from aperion import Aperion_parent_dir, settings
from aperion.engines import engines
...
def get_trace(traces):
    for trace in reversed(traces):
        ...
        if '/'.join(split_filename[-3:-1]) == 'aperion/engines':
            return trace
        if '/'.join(split_filename[-4:-1]) == 'aperion/search/processors':
            return trace
    return traces[-1]
...
def get_messages(exc, filename) -> typing.Tuple:  # pylint: disable=too-many-return-statements
    ...
    if isinstance(exc, AperionXPathSyntaxException):
        return (exc.xpath_str, exc.message)
    if isinstance(exc, AperionEngineXPathException):
        return (exc.xpath_str, exc.message)
    if isinstance(exc, AperionEngineAPIException):
        return (str(exc.args[0]),)
    if isinstance(exc, AperionEngineAccessDeniedException):
        return (exc.message,)
    return ()
...
def get_error_context(framerecords, exception_classname, log_message, log_parameters, secondary) -> ErrorContext:
    Aperion_frame = get_trace(framerecords)
    filename = Aperion_frame.filename
    if filename.startswith(Aperion_parent_dir):
        filename = filename[len(Aperion_parent_dir) + 1 :]
    function = Aperion_frame.function
    line_no = Aperion_frame.lineno
    code = Aperion_frame.code_context[0].strip()
    del framerecords
