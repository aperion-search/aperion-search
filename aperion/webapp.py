...
from aperion import infopage
from aperion import limiter
from aperion.botdetection import link_token, ProxyFix

from aperion.data import ENGINE_DESCRIPTIONS
from aperion.result_types import Answer
from aperion.settings_defaults import OUTPUT_FORMATS
from aperion.settings_loader import DEFAULT_SETTINGS_FILE
from aperion.exceptions import AperionParameterException
from aperion.engines import (
    DEFAULT_CATEGORY,
    categories,
    engines,
    engine_shortcuts,
...
from aperion import webutils
from aperion.webutils import (
    highlight_content,
...
from aperion.webadapter import (
    get_search_query_from_webapp,
    get_selected_categories,
    parse_lang,
)
from aperion.utils import gen_useragent, dict_subset
from aperion.version import VERSION_STRING, GIT_URL, GIT_BRANCH
from aperion.query import RawTextQuery
from aperion.plugins.oa_doi_rewrite import get_doi_resolver
from aperion.preferences import (
    Preferences,
    ClientPref,
    ValidationException,
)
...
from aperion.metrics import get_engines_stats, get_engine_errors, get_reliabilities, histogram, counter, openmetrics
from aperion.flaskfix import patch_application

from aperion.locales import (
    LOCALE_BEST_MATCH,
    LOCALE_NAMES,
    RTL_LOCALES,
    localeselector,
...
# renaming names from aperion imports ...
from aperion.autocomplete import search_autocomplete, backends as autocomplete_backends
from aperion import favicons

from aperion.valkeydb import initialize as valkey_initialize
from aperion.sxng_locales import sxng_locales
import aperion.search
from aperion.network import stream as http_stream, set_context_network_name
from aperion.search.checker import get_result as checker_get_result
...
def render(template_name: str, **kwargs):
    ...
    kwargs['Aperion_version'] = VERSION_STRING
    kwargs['Aperion_git_url'] = GIT_URL
...
def pre_request():
    ...
    preferences = Preferences(themes, list(categories.keys()), engines, aperion.plugins.STORAGE, client_pref)
    ...
    for plugin in aperion.plugins.STORAGE:
...
def index_error(output_format: str, error_message: str):
    ...
    if output_format == 'csv':
        ...
        cont_disp = 'attachment;Filename=aperion.csv'
...
def search():
    ...
    try:
        ...
        search_obj = aperion.search.SearchWithPlugins(search_query, sxng_request, sxng_request.user_plugins)
        result_container = search_obj.search()

    except AperionParameterException as e:
        logger.exception('search error: AperionParameterException')
    ...
    if output_format == 'csv':
        ...
        cont_disp = 'attachment;Filename=Aperion_-_{0}.csv'.format(search_query.query)
        response.headers.add('Content-Disposition', cont_disp)
        return response
    ...
    return render(
        ...
        # Enable ai_mode if the form requests it, or always enable since AI is always on
        ai_mode = (
            (sxng_request.form.get('ai_mode') == 'true') or True
        ),
        ai_config = {
            "enabled": True,
            "provider": "",
...
def generate_summary():
    ...
    api_key = ""
    num_results = 5
    timeout_per_result = 10
    max_tokens = 500
    system_prompt = ""
    timeout = 30 + (timeout_per_result * num_results)

    try:
        from aperion.ai_summarizer import generate_summary_with_fallback
        logger.info(f"Calling generate_summary_with_fallback with {len(results)} results, timeout={timeout}")
        ...
        logger.info(f"generate_summary_with_fallback result: success={result.get('success')}, error={result.get('error')}")
        return jsonify({
...
@app.route('/preferences/ai_models', methods=['POST'])
def ai_models():
    """Handle AI model list from preferences page."""
    if not flask.request.is_json:
        return jsonify({"success": False, "models": None, "error": "Content-Type must be application/json"}), 400
    ...
    try:
        ...
        if not models or not isinstance(models, list):
            return jsonify({"success": False, "models": None, "error": "Invalid models data"}), 400
        
        return jsonify({"success": True, "models": models})
    except Exception as e:
        logger.exception("Error processing AI models")
        return jsonify({"success": False, "models": None, "error": str(e)}), 500
...
def ai_chat():
    ...
    try:
        from aperion.ai_summarizer import generate_chat_with_fallback
        result = generate_chat_with_fallback(
            query=query,
            endpoint=endpoint,
            model=model,
            provider=provider,
        ...
        logger.exception("Error generating AI chat response")
        return jsonify({"success": False, "response": None, "stats": None, "error": str(e)}), 500


@app.route('/aperionai', methods=['GET'])
def aperionai():
    """Render a simple AI chat UI that uses the existing `/ai_chat` and `/generate_summary` endpoints."""
    q = flask.request.args.get('q', '')
    # Provide ai_config so the template can show info if needed
    ai_config = {
        "enabled": True,
        "provider": sxng_request.preferences.get_value("ai_provider") if sxng_request.preferences else "openai",
    ...
    return render('aperionai.html', q=q, ai_config=ai_config, current_locale=sxng_request.preferences.get_value('locale') if sxng_request.preferences else 'en')
...
def autocompleter():
    ...
    for obj in aperion.answerers.STORAGE.ask(sug_prefix):
    ...
    if len(raw_text_query.autocomplete_list) == 0 and len(sug_prefix) > 0:
        ...
        # get aperion's locale and autocomplete backend from cookie
    ...
    if sxng_request.headers.get('X-Requested-With') == 'XMLHttpRequest':
        # the suggestion request comes from the aperion search form
...
def preferences():
    ...
    return render(
        ...
        supports = supports,
        answer_storage = aperion.answerers.STORAGE.info,
        disabled_engines = disabled_engines,
        autocomplete_backends = autocomplete_backends,
        favicon_resolver_names = favicons.proxy.CFG.resolver_map.keys(),
        shortcuts = {y: x for x, y in engine_shortcuts.items()},
        themes = themes,
        plugins_storage = aperion.plugins.STORAGE.info,
...
def image_proxy():
    ...
    try:
        ...
        if resp and not forward_resp:
            ...
            # we make sure to close the response between aperion and the HTTP server
            try:
                resp.close()
            except httpx.HTTPError:
...
def engine_descriptions():
    ...
    for engine_name, engine_mod in engines.items():
        ...
        if descr is not None:
            result[engine_name] = [descr, "aperion config"]
...
def config():
    ...
    _plugins = []
    for _ in aperion.plugins.STORAGE:
    ...
    return jsonify(
        {
            ...
            'limiter': {
                ...
                'botdetection.ip_lists.pass_aperion_org': _limiter_cfg.get('botdetection.ip_lists.pass_aperion_org'),
            },
...
def run():
    ...
    This run method is only called when aperion is started via ``__main__``::

        python -m aperion.webapp
    ...
    command line script’s run support::
        ...
        flask --app aperion.webapp run --debug --reload --host 127.0.0.1 --port 8888
...
def init():
    ...
    if aperion.sxng_debug or app.debug:
        app.debug = True
        aperion.sxng_debug = True
    ...
    locales_initialize()
    valkey_initialize()
    aperion.plugins.initialize(app)

    metrics: bool = get_setting("general.enable_metrics")  # type: ignore
    aperion.search.initialize(enable_checker=True, check_network=True, enable_metrics=metrics)
