...
includes the configuration of the (:ref:`aperion appl <aperion settings.yml>`)
server. The default configuration for the application server is loaded from the
:origin:`DEFAULT_SETTINGS_FILE <aperion/settings.yml>`.  This default
configuration can be completely replaced or :ref:`customized individually
<use_default_settings.yml>` and the ``aperion_SETTINGS_PATH`` environment
...
- By default, local configurations are expected in folder ``/etc/aperion`` from
  where applications can load them with the :py:obj:`get_yaml_cfg` function.

- By default, customized :ref:`aperion appl <aperion settings.yml>` settings are
...
from aperion.exceptions import AperionSettingsException

Aperion_dir = os.path.abspath(os.path.dirname(__file__))

SETTINGS_YAML = Path("settings.yml")
DEFAULT_SETTINGS_FILE = Path(Aperion_dir) / SETTINGS_YAML
"""The :origin:`aperion/settings.yml` file with all the default settings."""
...
def load_yaml(file_name: str | Path):
    ...
    try:
        ...
        raise AperionSettingsException(e, str(file_name)) from e
...
def get_yaml_cfg(file_name: str | Path) -> dict:
    ...
    - in the ``aperion`` folder of the aperion installation
    """

    folder = get_user_cfg_folder() or Path(Aperion_dir)
...
def get_user_cfg_folder() -> Path | None:
    ...
    1. If the ``aperion_SETTINGS_PATH`` environment is set and points to a
       folder (e.g. ``/etc/mysxng/``), all local configurations are expected in
       this folder.  The settings of the :ref:`aperion appl <aperion
       settings.yml>` then expected in ``settings.yml``
       (e.g. ``/etc/mysxng/settings.yml``).

    2. If the ``aperion_SETTINGS_PATH`` environment is set and points to a file
       (e.g. ``/etc/mysxng/myinstance.yml``), this file contains the settings of
       the :ref:`aperion appl <aperion settings.yml>` and the folder
       (e.g. ``/etc/mysxng/``) is used for all other configurations.

       This type (``aperion_SETTINGS_PATH`` points to a file) is suitable for
       use cases in which different profiles of the :ref:`aperion appl <aperion
       settings.yml>` are to be managed, such as in test scenarios.

    3. If folder ``/etc/aperion`` exists, it is used.

    In case none of the above path exists, ``None`` is returned.  In case of
    environment ``aperion_SETTINGS_PATH`` is set, but the (folder or file) does
    not exists, a :py:obj:`EnvironmentError` is raised.

    """

    folder = None
    settings_path = os.environ.get("aperion_SETTINGS_PATH")

    # Disable default /etc/aperion is intended exclusively for internal testing purposes
    # and is therefore not documented!
    disable_etc = os.environ.get('aperion_DISABLE_ETC_SETTINGS', '').lower() in ('1', 'true')
...
def load_settings(load_user_settings=True) -> tuple[dict, str]:
    """Function for loading the settings of the aperion application
    (:ref:`settings.yml <aperion settings.yml>`)."""
    ...
    settings_yml = os.environ.get("aperion_SETTINGS_PATH")
    if settings_yml and Path(settings_yml).is_file():
        # see get_user_cfg_folder() --> aperion_SETTINGS_PATH points to a file
        settings_yml = Path(settings_yml).name
    else:
        # see get_user_cfg_folder() --> aperion_SETTINGS_PATH points to a folder
