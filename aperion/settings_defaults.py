...
Aperion_dir = abspath(dirname(__file__))

logger = logging.getLogger('aperion')
...
SCHEMA = {
    'general': {
        'debug': SettingsValue(bool, False, 'aperion_DEBUG'),
        'instance_name': SettingsValue(str, 'aperion'),
        'privacypolicy_url': SettingsValue((None, False, str), None),
        'contact_url': SettingsValue((None, False, str), None),
        'donation_url': SettingsValue((bool, str), "https://docs.aperion.org/donate.html"),
        'enable_metrics': SettingsValue(bool, True),
        'open_metrics': SettingsValue(str, ''),
    },
    'brand': {
        'issue_url': SettingsValue(str, 'https://github.com/aperion/aperion/issues'),
        'new_issue_url': SettingsValue(str, 'https://github.com/aperion/aperion/issues/new'),
        'docs_url': SettingsValue(str, 'https://docs.aperion.org'),
        'public_instances': SettingsValue((False, str), 'https://aperion.space'),
        'wiki_url': SettingsValue((False, str), 'https://github.com/aperion/aperion/wiki'),
    ...
    'search': {
        ...
        'suspended_times': {
            'AperionEngineAccessDenied': SettingsValue(numbers.Real, 86400),
            'AperionEngineCaptcha': SettingsValue(numbers.Real, 86400),
            'AperionEngineTooManyRequests': SettingsValue(numbers.Real, 3600),
            'cf_AperionEngineCaptcha': SettingsValue(numbers.Real, 1296000),
            'cf_AperionEngineAccessDenied': SettingsValue(numbers.Real, 86400),
            'recaptcha_AperionEngineCaptcha': SettingsValue(numbers.Real, 604800),
        },
        'formats': SettingsValue(list, OUTPUT_FORMATS),
        'max_page': SettingsValue(int, 0),
    },
    'server': {
        'port': SettingsValue((int, str), 8888, 'aperion_PORT'),
        'bind_address': SettingsValue(str, '127.0.0.1', 'aperion_BIND_ADDRESS'),
        'limiter': SettingsValue(bool, False, 'aperion_LIMITER'),
        'public_instance': SettingsValue(bool, False, 'aperion_PUBLIC_INSTANCE'),
        'secret_key': SettingsValue(str, environ_name='aperion_SECRET'),
        'base_url': SettingsValue((False, str), False, 'aperion_BASE_URL'),
        'image_proxy': SettingsValue(bool, False, 'aperion_IMAGE_PROXY'),
        'http_protocol_version': SettingsValue(('1.0', '1.1'), '1.0'),
        'method': SettingsValue(('POST', 'GET'), 'POST', 'aperion_METHOD'),
        'default_http_headers': SettingsValue(dict, {}),
    },
    # redis is deprecated ..
    'redis': {
        'url': SettingsValue((None, False, str), False, 'aperion_REDIS_URL'),
    },
    'valkey': {
        'url': SettingsValue((None, False, str), False, 'aperion_VALKEY_URL'),
    },
    'ui': {
        'static_path': SettingsDirectoryValue(str, os.path.join(Aperion_dir, 'static')),
        'templates_path': SettingsDirectoryValue(str, os.path.join(Aperion_dir, 'templates')),
