...
from aperion.extended_types import SXNG_Response
from aperion import settings
from aperion.engines import (
    engines,
    google,
)
from aperion.network import get as http_get, post as http_post
from aperion.exceptions import AperionEngineResponseException
from aperion.utils import extr, gen_useragent
...
def google_complete(query, sxng_locale):
    ...
    (:py:obj:`aperion.engines.google.get_google_info`) by using the async REST
    API::
    ...
    google_info = google.get_google_info({'aperion_locale': sxng_locale}, engines['google'].traits)
...
def sogou(query, _lang):
    ...
    response = get(base_url + urlencode({'m': 'aperion', 'key': query}))
...
def search_autocomplete(backend_name, query, sxng_locale):
    ...
    except (HTTPError, AperionEngineResponseException):
        return []
