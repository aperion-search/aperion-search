...
from aperion.utils import gen_useragent
...
from aperion.metrics.error_recorder import count_error
...
class OnlineProcessor(EngineProcessor):
    ...
    def initialize(self):
        ...
        aperion.network.set_timeout_for_thread(self.engine.timeout, start_time=default_timer())
        # reset the HTTP total time
        aperion.network.reset_time_for_thread()
        # set the network
        aperion.network.set_context_network_name(self.engine_name)
        super().initialize()
    ...
    def _send_http_request(self, params):
        ...
        if params['method'] == 'GET':
            req = aperion.network.get
        else:
            req = aperion.network.post
    ...
    def search(self, query, params, result_container, start_time, timeout_limit):
        # set timeout for all HTTP requests
        aperion.network.set_timeout_for_thread(timeout_limit, start_time=start_time)
        # reset the HTTP total time
        aperion.network.reset_time_for_thread()
        # set the network
        aperion.network.set_context_network_name(self.engine_name)
        ...
        try:
            ...
            self.logger.error("SSLError {}, verify={}".format(e, aperion.network.get_network(self.engine_name).verify))
        ...
        except AperionEngineAccessDeniedException as e:
            self.handle_exception(result_container, e, suspend=True)
            self.logger.exception('aperion is blocked')
