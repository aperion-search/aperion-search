...
import aperion.search.checker
from aperion.search import PROCESSORS
from aperion.engines import engine_shortcuts
...
def iter_processor(engine_name_list):
    if len(engine_name_list) > 0:
        ...
        for name, processor in aperion.search.PROCESSORS.items():
            yield name, processor
...
def run(engine_name_list, verbose):
    ...
    for name, processor in iter_processor(engine_name_list):
        ...
        checker = aperion.search.checker.Checker(processor)
        checker.run()
...
# call by setup.py
def main():
    parser = argparse.ArgumentParser(description='Check aperion engines.')
    parser.add_argument(
        'engine_name_list',
        metavar='engine name',
        type=str,
