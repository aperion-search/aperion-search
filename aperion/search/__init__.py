...
from aperion.engines import load_engines
from aperion.extended_types import SXNG_Request
from aperion.external_bang import get_bang_url
from aperion.metrics import initialize as initialize_metrics, counter_inc, histogram_observe_time
from aperion.network import initialize as initialize_network, check_network_configuration
from aperion.results import ResultContainer
from aperion.search.checker import initialize as initialize_checker
from aperion.search.models import SearchQuery
from aperion.search.processors import PROCESSORS, initialize as initialize_processors
...
class Search:
    ...
    def search_answerers(self):
        ...
        results = aperion.answerers.STORAGE.ask(self.search_query.query)
...
class SearchWithPlugins(Search):
    ...
    def _on_result(self, result):
        return aperion.plugins.STORAGE.on_result(self.request, self, result)

    def search(self) -> ResultContainer:

        if aperion.plugins.STORAGE.pre_search(self.request, self):
            super().search()

        aperion.plugins.STORAGE.post_search(self.request, self)
