...
from aperion.utils import extract_text, eval_xpath_list, eval_xpath_getindex
from aperion.enginelib.traits import EngineTraits
from aperion.locales import language_tag
...
def request(query, params):
    ...
    sxng_lang = params['aperion_locale'].split('-')[0]
    ...
    if netloc == main_wiki:
        ...
        # - https://github.com/aperion/aperion/issues/4646#issuecomment-2817848019
        params['headers']['User-Agent'] = "aperion"
...
def response(resp):
    ...
    sxng_lang = resp.search_params['aperion_locale'].split('-')[0]
    netloc: str = traits.custom['wiki_netloc'].get(sxng_lang, main_wiki)  # type: ignore
    base_url = 'https://' + netloc + '/index.php?'
...
def fetch_traits(engine_traits: EngineTraits):
    ...
    <aperion.enginelib.traits.EngineTraits.custom>` (``wiki_netloc``).  Depending
