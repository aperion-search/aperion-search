...
from aperion.network import get, raise_for_httperror  # see https://github.com/aperion/aperion/issues/762
from aperion.utils import html_to_text
...
aperion needs in the response to build a video result list.
"""
...
"""URL template to embed video in aperion's result list."""
...
def request(query, params):
    ...
    eng_region: str = traits.get_region(params['aperion_locale'], 'en_US')  # type: ignore
    eng_lang = traits.get_language(params['aperion_locale'], 'en')
    ...
    if len(params['aperion_locale'].split('-')) > 1:
...
def response(resp):
    ...
    if 'error' in search_res:
        raise AperionEngineAPIException(search_res['error'].get('message'))
...
def fetch_traits(engine_traits: EngineTraits):
    ...
    .. _PR1071: https://github.com/aperion/aperion/pull/1071
