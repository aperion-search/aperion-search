...
With this options a aperion maintainer is able to configure **additional**
...
from aperion.utils import extract_text, eval_xpath, eval_xpath_list
from aperion.enginelib.traits import EngineTraits
from aperion.data import ENGINE_TRAITS
from aperion.exceptions import AperionException
...
def response(resp: httpx.Response) -> List[Dict[str, Any]]:
    ...
    if domain_is_seized(dom):
        raise AperionException(f"zlibrary domain is seized: {base_url}")
...
def fetch_traits(engine_traits: EngineTraits) -> None:
    ...
    except (AperionException, httpx.HTTPError) as exc:
