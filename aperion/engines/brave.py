...
Brave uses two-digit tags for the regions like ``ca`` while aperion deals with
locales.  To get a mapping, all *officiat de-facto* languages of the Brave
region are mapped to regions in aperion (see :py:obj:`babel
...
In the :py:obj:`EngineTraits object <aperion.enginelib.traits.EngineTraits>` the
...
in aperion, the spellchecking is disabled by default.
"""
...
def request(query, params):
    ...
    # Don't accept br encoding / see https://github.com/aperion/aperion/pull/1787
    ...
    engine_region = traits.get_region(params['aperion_locale'], 'all')
    params['cookies']['country'] = engine_region.split('-')[-1].lower()  # type: ignore

    ui_lang = locales.get_engine_locale(params['aperion_locale'], traits.custom["ui_lang"], 'en-us')
...
def fetch_traits(engine_traits: EngineTraits):
    ...
    from aperion.locales import region_tag, language_tag
