...
video streams aperion can't use the video template for this and if aperion can't
...
from aperion.network import get
from aperion.utils import gen_useragent, html_to_text, parse_duration_string
...
def _get_request_id(query, params):
    ...
    if params['aperion_locale'] != 'all':
        l = locales.get_locale(params['aperion_locale'])

        # Presearch narrows down the search by region.  In aperion when the user
        # does not set a region (e.g. 'en-CA' / canada) we cannot hand over a region.

        # We could possibly use aperion.locales.get_official_locales to determine
...
def response(resp):
    ...
    if search_type == 'search':
        ...
        # a video and not to a video stream --> aperion can't use the video template.
