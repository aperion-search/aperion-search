...
.. _issue #4694: https://github.com/aperion/aperion/issues/4694
...
def response(resp):
    try:
        ...
        raise AperionEngineAPIException(f"Invalid response: {e}") from e
...
def parse_news(data):
    ...
    if not data.get("data", {}).get("data"):
        raise AperionEngineAPIException("Invalid response")
