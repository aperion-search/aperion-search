...
from aperion.utils import (
    eval_xpath,
    eval_xpath_getindex,
    extr,
    extract_text,
)
from aperion.network import get  # see https://github.com/aperion/aperion/issues/762
from aperion.enginelib.traits import EngineTraits
from aperion.enginelib import EngineCache
from aperion.exceptions import AperionEngineCaptchaException
...
def get_vqd(query: str, region: str, force_request: bool = False) -> str:
    ...
    If aperion cannot determine a ``vqd`` value, then no request should go out
    to DDG.

    .. attention::

       A request with a wrong ``vqd`` value leads to DDG temporarily putting
       aperion's IP on a block list.
...
def get_ddg_lang(eng_traits: EngineTraits, sxng_locale, default='en_US'):
    """Get DuckDuckGo's language identifier from aperion's locale.
    ...
    .. code: python
       ...
       eng_region = traits.get_region(params['aperion_locale'], traits.all_locale)
       eng_lang = get_ddg_lang(traits, params['aperion_locale'])

    It might confuse, but the ``l`` value of the cookie is what aperion calls
    the *region*:
...
def request(query, params):
    ...
    eng_region: str = traits.get_region(params['aperion_locale'], traits.all_locale)  # type: ignore
    ...
    if params['pageno'] == 1:
        ...
        if vqd:
            ...
            # reputation of the aperion IP and DDG starts to activate CAPTCHAs.
            params["url"] = None
            return

        if params['aperion_locale'].startswith("zh"):
...
def response(resp) -> EngineResults:
    ...
    if is_ddg_captcha(doc):
        ...
        raise AperionEngineCaptchaException(suspended_time=0, message=f"CAPTCHA ({resp.search_params['data'].get('kl')})")
...
def fetch_traits(engine_traits: EngineTraits):
    ...
    aperion's ``all`` locale maps DuckDuckGo's "Alle regions" (``wt-wt``).
    DuckDuckGo's language "Browsers preferred language" (``wt_WT``) makes no
    sense in a aperion request since aperion's ``all`` will not add a
    ...
    from aperion.utils import js_variable_to_python
