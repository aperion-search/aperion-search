...
# fallback values
# if there is aperion.version_frozen module, and it is not possible to get the git tag
...
def get_git_version():
    ...
    # add "+dirty" suffix if there are uncommitted changes except aperion/settings.yml
    try:
        subprocess_run("git diff --quiet -- . ':!aperion/settings.yml' ':!utils/brand.env'")
...
try:
    vf = importlib.import_module('aperion.version_frozen')
...
# this file is generated automatically by aperion/version.py
...
"""
        ...
        # usage: eval "$(python -m aperion.version)"
        shell_code = f"""
VERSION_STRING="{VERSION_STRING}"
VERSION_TAG="{VERSION_TAG}"
DOCKER_TAG="{DOCKER_TAG}"
