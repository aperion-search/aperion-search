...
weblate.help() {
    cat <<EOF
weblate.:
  push.translations: push translation changes from aperion to Weblate's counterpart
...
weblate.translations.worktree() {
    ...
    # 'translations' from Weblate's counterpart (weblate) of the aperion
    # (origin).
    #
    #     remote weblate https://translate.codeberg.org/git/aperion/aperion/

    (
        set -e
        if ! git remote get-url weblate 2>/dev/null; then
            git remote add weblate https://translate.codeberg.org/git/aperion/aperion/
        fi
...
weblate.to.translations() {
    ...
    # Update 'translations' branch of aperion (origin) with last additions from
    # Weblate.
...
weblate.translations.commit() {
    ...
    (
        ...
        cp -rv --preserve=mode,timestamps "${TRANSLATIONS_WORKTREE}/aperion/translations" "aperion"

        # compile translations
        build_msg BABEL 'compile translation catalogs into binary MO files'
        pybabel compile --statistics \
            -d "aperion/translations"

        # update aperion/data/translation_labels.json
        data.locales
        ...
        git add aperion/translations
        git add aperion/data/locales.json
...
weblate.push.translations() {
    ...
    # Push *translation changes* from aperion (origin) to Weblate's counterpart
    # (weblate).

    # In branch master of aperion (origin) check for meaningful changes in
    # folder 'aperion/translations', commit changes on branch 'translations' and
    ...
    # 3. Update 'translations' branch of aperion (origin) with last additions
    #    from Weblate.
    ...
    local messages_pot diff_messages_pot last_commit_hash last_commit_detail \
        exitcode
    messages_pot="${TRANSLATIONS_WORKTREE}/aperion/translations/messages.pot"
    (
        set -e
        pyenv.activate
        # get translations branch in git worktree (TRANSLATIONS_WORKTREE)
        ...
        pybabel extract -F babel.cfg --project="aperion" --version="-" \
            -o "${messages_pot}" \
            "aperion/"
        ...
        diff_messages_pot=$(
            cd "${TRANSLATIONS_WORKTREE}"
            git diff -- "aperion/translations/messages.pot"
        )
    ...
    (
        ...
        pybabel update -N \
            -i "${messages_pot}" \
            -d "${TRANSLATIONS_WORKTREE}/aperion/translations"
