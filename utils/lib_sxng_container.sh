...
CONTAINER_IMAGE_ORGANIZATION=${GITHUB_REPOSITORY_OWNER:-"aperion"}
CONTAINER_IMAGE_NAME="aperion"
...
container.build() {
    ...
    (
        ...
        python -m aperion.version freeze
        eval "$(python -m aperion.version)"
        ...
        "$container_engine" $params_build_builder \
            --build-arg="TIMESTAMP_SETTINGS=$(git log -1 --format="%cd" --date=unix -- ./aperion/settings.yml)" \
