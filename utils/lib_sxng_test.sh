...
test.:
  ...
  pylint    : lint ./aperion, ./aperion_extra and ./tests
...
test.pylint() {
    (
        ...
        build_msg TEST "[pylint] ./aperion/engines"
        ...
        build_msg TEST "[pylint] ./aperion ./aperion_extra ./tests"
        # shellcheck disable=SC2086
        pylint ${PYLINT_OPTIONS} ${PYLINT_VERBOSE} \
            --ignore=aperion/engines \
            aperion aperion/aperion.msg \
            aperion_extra aperion_extra/docs_prebuild \
            tests
    )
    dump_return $?
}
...
test.pyright() {
    ...
    # we need to work on aperion's typification.
...
test.coverage() {
    ...
    (
        ...
        pyenv.activate
        # shellcheck disable=SC2086
        python -m nose2 ${TEST_NOSE2_VERBOSE} -C --log-capture --with-coverage --coverage aperion -s tests/unit
        coverage report
        coverage html
    )
    dump_return $?
...
test.pybabel() {
    ...
    pyenv.cmd pybabel extract -F babel.cfg -o "${TEST_BABEL_FOLDER}/messages.pot" aperion
}
