...
os.environ['aperion_DEBUG'] = '1'

# from here on implement the checks of the installation

import aperion

OLD_SETTING = '/etc/aperion/settings.yml'

if os.path.isfile(OLD_SETTING):
    msg = (
        '%s is no longer valid, move setting to %s' % (
            OLD_SETTING,
            os.environ.get('aperion_SETTINGS_PATH', '/etc/aperion/settings.yml')
        ))
...
from aperion import valkeydb, get_setting
