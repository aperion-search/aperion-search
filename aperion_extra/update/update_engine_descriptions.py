...
:origin:`aperion/engines/wikidata.py` engine.

Output file: :origin:`aperion/data/engine_descriptions.json`.
...
from aperion.engines import wikidata, set_loggers
from aperion.utils import extract_text, aperion_useragent
from aperion.locales import LOCALE_NAMES, locales_initialize, match_locale
from aperion import Aperion_dir
from aperion.utils import gen_useragent, detect_language
import aperion.search
import aperion.network
from aperion.data import data_dir
...
def get_wikipedia_summary(wikipedia_url, aperion_locale):
    # get the REST API URL from the HTML URL

    # Headers
    headers = {'User-Agent': aperion_useragent()}

    if aperion_locale in WIKIPEDIA_LANGUAGE_VARIANTS:
        headers['Accept-Language'] = WIKIPEDIA_LANGUAGE_VARIANTS.get(aperion_locale)
    ...
    try:
        response = aperion.network.get(wikipedia_rest_url, headers=headers, timeout=10)
...
def get_website_description(url, lang1, lang2=None):
    ...
    try:
        response = aperion.network.get(url, headers=headers, timeout=10)
        response.raise_for_status()
...
def initialize():
    ...
    wikipedia_engine = aperion.engines.engines['wikipedia']
    ...
    for engine_name, engine in aperion.engines.engines.items():
...
def fetch_wikidata_descriptions():
    ...
    aperion.network.set_timeout_for_thread(60)
    ...
    if result is not None:
        for binding in result['results']['bindings']:
            ...
            for engine_name in wd_to_engine_name[wikidata_id]:
                for aperion_locale in LOCALE_NAMES:
                    if WIKIPEDIA_LANGUAGES[aperion_locale] != wikidata_lang:
                        continue
                    print(
                        f"    engine: {engine_name:20} / wikidata_lang: {wikidata_lang:5}",
                        f"/ len(wikidata_desc): {len(desc)}",
                    )
                    update_description(engine_name, aperion_locale, desc, 'wikidata')
        ...
        for binding in result['results']['bindings']:
            ...
            for engine_name in wd_to_engine_name[wikidata_id]:
                for aperion_locale in LOCALE_NAMES:
                    if WIKIPEDIA_LANGUAGES[aperion_locale] != wikidata_lang:
                        continue
                    desc = get_wikipedia_summary(wikipedia_url, aperion_locale)
                    if not desc:
                        continue
                    print(
                        f"    engine: {engine_name:20} / wikidata_lang: {wikidata_lang:5}",
                    ...
                    update_description(engine_name, aperion_locale, desc, 'wikipedia')
...
def get_engine_descriptions_filename():
    return join(join(Aperion_dir, "data"), "engine_descriptions.json")
