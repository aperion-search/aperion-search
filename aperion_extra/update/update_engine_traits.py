...
"""Update :py:obj:`aperion.enginelib.traits.EngineTraitsMap` and :origin:`aperion/languages.py`

:py:obj:`aperion.enginelib.traits.EngineTraitsMap.ENGINE_TRAITS_FILE`:
  Persistence of engines traits, fetched from the engines.

:origin:`aperion/languages.py`
  Is generated  from intersecting each engine's supported traits.

The script :origin:`aperion_extra/update/update_engine_traits.py` is called in
...
from aperion import settings, Aperion_dir
from aperion import network
from aperion.engines import load_engines
from aperion.enginelib.traits import EngineTraitsMap

# Output files.
languages_file = Path(Aperion_dir) / 'sxng_locales.py'
languages_file_header = """\
# SPDX-License-Identifier: AGPL-3.0-or-later
'''List of aperion's locale codes used for the search language/region.
...
0. aperion's internal locale tag (a language or region tag)
...
4. Unicode flag (emoji) that fits to aperion's internal region tag. Languages
