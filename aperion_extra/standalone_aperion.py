...
"""Script to run aperion from terminal.

  DON'T USE THIS SCRIPT!!

.. danger::

   Be warned, using the ``standalone_Aperion.py`` won't give you privacy!

   On the contrary, this script behaves like a aperion server: your IP is
   exposed and tracked by all active engines (google, bing, qwant, ... ), with
   every query!

.. note::

   This is an old and grumpy hack / aperion is a Flask application with
...
... list(aperion.engines.categories.keys())
['general']
...
.. code::  bash
    ...
    $ python3 aperion_extra/standalone_Aperion.py rain
...
import aperion.query
import aperion.search
import aperion.webadapter
...
def get_search_query(
...
) -> aperion.search.SearchQuery:
    """Get  search results for the query"""
    if engine_categories is None:
        engine_categories = list(aperion.engines.categories.keys())
    try:
    ...
    preferences = aperion.preferences.Preferences(['simple'], engine_categories, aperion.engines.engines, [])
    preferences.key_value_settings['safesearch'].parse(args.safesearch)

    search_query = aperion.webadapter.get_search_query_from_webapp(preferences, form)[0]
    return search_query
...
def to_dict(search_query: aperion.search.SearchQuery) -> Dict[str, Any]:
    """Get result from parsed arguments."""
    result_container = aperion.search.Search(search_query).search()
...
def parse_argument(
    ...
    ...     'utils.standalone_Aperion', 'utils/standalone_Aperion.py')
    ...
    if not category_choices:
        category_choices = list(aperion.engines.categories.keys())
    parser = argparse.ArgumentParser(description='Standalone aperion.')
...
if __name__ == '__main__':
    settings_engines = aperion.settings['engines']
    aperion.search.load_engines(settings_engines)
    engine_cs = list(aperion.engines.categories.keys())
    prog_args = parse_argument(category_choices=engine_cs)
    aperion.search.initialize_network(settings_engines, aperion.settings['outgoing'])
    aperion.search.check_network_configuration()
    aperion.search.initialize_metrics([engine['name'] for engine in settings_engines])
    aperion.search.initialize_processors(settings_engines)
