...
import aperion.ai_summarizer
import aperion.search
import aperion.search.processors
from aperion.result_types._base import MainResult

from aperion.results import Timing
from aperion.preferences import Preferences
from tests import AperionTestCase


class ViewsTestCase(AperionTestCase):  # pylint: disable=too-many-public-methods
    ...
    def setUp(self):
        ...
        self.setattr4test(aperion.search.processors, 'initialize_processor', dummy)
        ...
        self.setattr4test(aperion.search.Search, 'search', search_mock)
    ...
    def test_index_empty(self):
        ...
        self.assertIn(
            b'<div class="title"><h1>aperion</h1></div>',
            result.data,
        )
    ...
    def test_search_empty_html(self):
        ...
        self.assertEqual(result.status_code, 200)
        self.assertIn(b'<div class="title"><h1>aperion</h1></div>', result.data)
    ...
    def test_search_uses_summary_endpoint_when_ai_summarization_enabled(self):
        original_preferences_get_value = Preferences.get_value

        def preferences_get_value(preferences_self, user_setting_name: str):
            if user_setting_name == 'theme':
                return 'simple'
            if user_setting_name == 'ai_summarization_enabled':
                return True
            return original_preferences_get_value(preferences_self, user_setting_name)

        self.setattr4test(Preferences, 'get_value', preferences_get_value)

        result = self.client.post('/search', data={'q': 'test', 'ai_mode': 'true'})

        self.assertEqual(result.status_code, 200)
        self.assertIn(b"fetch('/generate_summary'", result.data)

    def test_home_page_search_form_has_ai_toggle(self):
        result = self.client.get('/')

        self.assertEqual(result.status_code, 200)
        self.assertIn(b'id="ai_mode_toggle"', result.data)
        self.assertIn(b'id="ai_mode_input"', result.data)

    def test_generate_summary_sync_retries_with_chat_completions_path(self):
        class FakeResponse:
        ...
        with patch('aperion.ai_summarizer.httpx.Client', return_value=fake_client):
            result = aperion.ai_summarizer.generate_summary_sync(
                results=[{"title": "Test", "content": "Test content", "url": "http://example.com"}],
                query="test",
    ...
    def test_generate_summary_sync_handles_gemini_provider(self):
        class FakeResponse:
        ...
        self.assertTrue(result["success"])
        self.assertEqual(result["summary"], "Gemini summary")
        self.assertEqual(len(fake_client.calls), 1)
        self.assertIn('models/gemini-1.5-flash:generateContent', fake_client.calls[0])

    def test_generate_summary_sync_normalizes_gemini_model_name(self):
        class FakeResponse:
            def __init__(self, payload):
                self._payload = payload
    ...
    def test_search_rss(self):
        ...
        self.assertIn(b'<description>Search results for "test" - aperion</description>', result.data)
    ...
    def test_opensearch_xml(self):
        ...
        self.assertEqual(result.status_code, 200)
        self.assertIn(
            b'<Description>aperion is a metasearch engine that respects your privacy.</Description>', result.data
        )
