...
from aperion import settings, engines
from tests import AperionTestCase


class TestEnginesInit(AperionTestCase):
    ...
    def test_missing_name_field(self):
        ...
        with self.assertLogs('aperion.engines', level='ERROR') as cm:  # pylint: disable=invalid-name
            engines.load_engines(engine_list)
            self.assertEqual(len(engines.engines), 0)
            self.assertEqual(cm.output, ['ERROR:aperion.engines:An engine does not have a "name" field'])
    ...
    def test_missing_engine_field(self):
        ...
        with self.assertLogs('aperion.engines', level='ERROR') as cm:  # pylint: disable=invalid-name
            ...
            self.assertEqual(len(engines.engines), 0)
            self.assertEqual(
                cm.output, ['ERROR:aperion.engines:The "engine" field is missing for the engine named "engine2"']
            )
