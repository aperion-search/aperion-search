...
plg_store = aperion.plugins.PluginStorage()
plg_store.load_settings(aperion.get_setting("plugins"))
...
def get_search_mock(query, **kwargs):
    ...
    return Mock(
        ...
        result_container=aperion.results.ResultContainer(),
    )
...
class PluginMock(aperion.plugins.Plugin):

    def __init__(self, _id: str, name: str, active: bool):
        plg_cfg = aperion.plugins.PluginCfg(active=active)
        self.id = _id
        self._name = name
    ...
    def info(self):
        return aperion.plugins.PluginInfo(
            id=self.id,
            name=self._name,
...
class PluginStorage(AperionTestCase):
