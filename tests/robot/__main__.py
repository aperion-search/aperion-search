...
import tests as aperion_tests
from tests.robot import test_webapp


class AperionRobotLayer:
    """aperion Robot Test Layer"""

    def setUp(self):
        os.setpgrp()  # create new process group, become its leader

        tests_path = pathlib.Path(aperion_tests.__file__).resolve().parent

        # get program paths
        webapp = str(tests_path.parent / 'aperion' / 'webapp.py')
        exe = 'python'

        # set robot settings path
        os.environ['aperion_SETTINGS_PATH'] = str(tests_path / 'robot' / 'settings_robot.yml')
    ...
    def tearDown(self):
        ...
        del os.environ['aperion_SETTINGS_PATH']
