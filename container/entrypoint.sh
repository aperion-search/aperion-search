...
EOF
    ...
    if [ "$target_ownership" != "aperion:aperion" ]; then
        if [ "${FORCE_OWNERSHIP:-true}" = true ] && [ "$(id -u)" -eq 0 ]; then
            chown -R aperion:aperion "$target"
        else
            cat <<EOF
!!!
!!! WARNING
!!! "$target" $type is not owned by "aperion:aperion"
!!! This may cause issues when running aperion
!!!
!!! Expected "aperion:aperion"
...
# Check for files
config_handler "$aperion_SETTINGS_PATH" "/usr/local/aperion/aperion/settings.yml"

exec /usr/local/aperion/.venv/bin/granian aperion.webapp:app
