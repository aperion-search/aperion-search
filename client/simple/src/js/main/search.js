...
((_w, d, aperion) => {
  ...
  const fetchResults = async (query) => {
    let request;
    if (aperion.settings.method === "GET") {
    ...
    request.then(async (response) => {
      ...
      if (!results[1] || results[1].length === 0) {
        ...
        noItemFoundMessage.innerHTML = aperion.settings.translations.no_item_found;
  ...
  aperion.ready(() => {
    ...
    if (qinput !== null) {
      ...
      // autocompleter
      if (aperion.settings.autocomplete) {
        aperion.on(qinput, "input", () => {
          const query = qinput.value;
          if (query.length < aperion.settings.autocomplete_min) return;
    ...
    if (
      qinput !== null &&
      aperion.settings.search_on_category_select &&
      // If .search_filters is undefined (invisible) we are on the homepage and
      // hence don't have to set any listeners
      d.querySelector(".search_filters") != null
    ) {
      aperion.on(d.getElementById("safesearch"), "change", submitIfQuery);
      aperion.on(d.getElementById("time_range"), "change", submitIfQuery);
      aperion.on(d.getElementById("language"), "change", submitIfQuery);
    }

    const categoryButtons = d.querySelectorAll("button.category_button");
    for (const button of categoryButtons) {
      aperion.on(button, "click", (event) => {
        if (event.shiftKey) {
