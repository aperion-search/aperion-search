...
``request_timeout`` :
  ...
  will slow aperion reactivity (the result page may take the time specified in the
  timeout to load).  Can be override by ``timeout`` in the :ref:`settings engines`.

``useragent_suffix`` :
  Suffix to the user-agent aperion uses to send requests to others engines.  If an
...
``retries`` :
  Number of retry in case of an HTTP error.  On each retry, aperion uses an
...
``verify``: : ``$SSL_CERT_FILE``, ``$SSL_CERT_DIR``
  ...
  In addition to ``verify``, aperion supports the ``$SSL_CERT_FILE`` (for a file) and
