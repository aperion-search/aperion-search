...
.. sidebar:: Update the OS first!
   ...
   To avoid unwanted side effects, update your OS before installing aperion.
...
Jump to a folder that is readable by *others* and start to clone aperion,
alternatively you can create your own fork and clone from there.

.. code:: bash

   $ cd ~/Downloads
   $ git clone https://github.com/aperion/aperion.git aperion
   $ cd aperion
...
To install a aperion :ref:`reference setup <use_default_settings.yml>`
including a :ref:`uWSGI setup <architecture uWSGI>` as described in the
:ref:`installation basic` and in the :ref:`aperion uwsgi` section type:

.. code:: bash

   $ sudo -H ./utils/aperion.sh install all
...
.. attention::
   ...
   ``aperion`` user.  In the installation procedure this new created user does
   need to have read access to the cloned aperion repository, which is not the case if you clone
...
When all services are installed and running fine, you can add aperion to your
