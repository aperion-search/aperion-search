...
This section explains how to set up a aperion instance using the HTTP server Apache_.
If you did use the :ref:`installation scripts` and do not have any special preferences
you can install the :ref:`aperion site <apache aperion site>` using
:ref:`aperion.sh <aperion.sh overview>`:

.. code:: bash

   $ sudo -H ./utils/aperion.sh install apache
...
.. tabs::
   ...
   .. group-tab:: Ubuntu / debian
      ...
      In :ref:`Debian's Apache layout` you create a ``aperion.conf`` with the
      ``<Location /aperion >`` directive and save this file in the *sites
      available* folder at ``/etc/apache2/sites-available``.  To enable the
      ``aperion.conf`` use :man:`a2ensite`:

      .. code:: bash

         sudo -H a2ensite aperion.conf
   ...
   .. group-tab:: Arch Linux
      ...
      .. code:: bash
         ...
         sudo -H ln -s /etc/httpd/sites-available/aperion.conf \
                       /etc/httpd/sites-enabled/aperion.conf
...
.. _apache aperion site:

Apache's aperion site
=====================
...
To proxy the incoming requests to the aperion instance Apache needs the
...
Depending on what your aperion installation is listening on, you need a http
...
You can install and activate your own ``aperion.conf`` like shown in
:ref:`apache sites`.
...
.. code:: apache
   ...
   SetEnvIf Request_URI "/aperion" dontlog
   # CustomLog /dev/null combined env=dontlog

The ``CustomLog`` directive disables logs for the entire (virtual) server, use it
when the URL of the service does not have a path component (``/aperion``), so when
aperion is located at root (``/``).
