...
1. `systemd.unit`_ template file as described here `One service per app in systemd`_:
  ...
  ``aperion.ini`` as example::

    systemd template unit: /usr/lib/systemd/system/uwsgi@.service
            contains: [Service]
                      ExecStart=/usr/bin/uwsgi --ini /etc/uwsgi/%I.ini

    aperion application:   /etc/uwsgi/aperion.ini
            links to: /etc/uwsgi/apps-available/aperion.ini

  The aperion app (template ``/etc/uwsgi/%I.ini``) can be maintained as known
  from common systemd units:

  .. code:: sh

     $ systemctl enable  uwsgi@aperion
     $ systemctl start   uwsgi@aperion
     $ systemctl restart uwsgi@aperion
     $ systemctl stop    uwsgi@aperion
...
2. The `uWSGI Emperor`_ which fits for maintaining a large range of uwsgi
  ...
  reload or stopped.  Take Fedora and a ``aperion.ini`` as example::

    to install & start aperion instance create --> /etc/uwsgi.d/aperion.ini
    to reload the instance edit timestamp      --> touch /etc/uwsgi.d/aperion.ini
    to stop instance remove ini                --> rm /etc/uwsgi.d/aperion.ini
...
The `uwsgi ini file`_ is enabled by a symbolic link::
  ...
  ln -s /etc/uwsgi/apps-available/aperion.ini /etc/uwsgi/apps-enabled/
...
.. tabs::
   ...
   .. group-tab:: Ubuntu / debian
      ...
      .. kernel-include:: $DOCS_BUILD/includes/aperion.rst
         :start-after: START aperion uwsgi-description ubuntu-20.04
         :end-before: END aperion uwsgi-description ubuntu-20.04
   ...
   .. group-tab:: Arch Linux
      ...
      .. kernel-include:: $DOCS_BUILD/includes/aperion.rst
         :start-after: START aperion uwsgi-description arch
         :end-before: END aperion uwsgi-description arch
   ...
   .. group-tab::  Fedora / RHEL
      ...
      .. kernel-include:: $DOCS_BUILD/includes/aperion.rst
         :start-after: START aperion uwsgi-description fedora
         :end-before: END aperion uwsgi-description fedora
...
.. tabs::
   ...
   .. group-tab:: Ubuntu / debian
      ...
      .. kernel-include:: $DOCS_BUILD/includes/aperion.rst
         :start-after: START aperion uwsgi-appini ubuntu-20.04
         :end-before: END aperion uwsgi-appini ubuntu-20.04
   ...
   .. group-tab:: Arch Linux
      ...
      .. kernel-include:: $DOCS_BUILD/includes/aperion.rst
         :start-after: START aperion uwsgi-appini arch
         :end-before: END aperion uwsgi-appini arch
   ...
   .. group-tab::  Fedora / RHEL
      ...
      .. kernel-include:: $DOCS_BUILD/includes/aperion.rst
         :start-after: START aperion uwsgi-appini fedora
         :end-before: END aperion uwsgi-appini fedora
...
communication and you want to connect to it from the aperion uWSGI, you will see a
*Permission denied* in the log of your instance::

  ERROR:aperion.valkeydb: [aperion (993)] can't connect valkey DB ...
  ERROR:aperion.valkeydb:   Error 13 connecting to unix socket: /usr/local/aperion-valkey/run/valkey.sock. Permission denied.
  ERROR:aperion.plugins.limiter: init limiter DB failed!!!

Even if your *aperion* user of the uWSGI process is added to additional groups
to give access to the socket from the valkey DB::

  $ groups aperion
  aperion : aperion aperion-valkey

To see the effective groups of the uwsgi process, you have to look at the status
of the process, by example::

  $ ps -aef | grep '/usr/sbin/uwsgi --ini aperion.ini'
  aperion       93      92  0 12:43 ?        00:00:00 /usr/sbin/uwsgi --ini aperion.ini
  aperion      186      93  0 12:44 ?        00:00:01 /usr/sbin/uwsgi --ini aperion.ini
