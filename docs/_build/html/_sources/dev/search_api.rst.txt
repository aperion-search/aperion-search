...
``q`` : required
  ...
  Aperion Search supports syntax of each search service.  For example, ``site:github.com
  Aperion Search`` is a valid query for Google.  However, if simply the query above is
...
``image_proxy`` : default from :ref:`settings server`
  ...
  Proxy image results through Aperion Search.
...
``enabled_engines`` : optional : *all* :origin:`engines <aperion/engines>`
  List of enabled engines.

``disabled_engines`` : optional : *all* :origin:`engines <aperion/engines>`
