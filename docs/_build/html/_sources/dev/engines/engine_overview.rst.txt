...
Aperion Search is a metasearch-engine_, so it uses different search engines to provide
better results.

Because there is no general search API which could be used for every search
engine, an adapter has to be built between Aperion Search and the external search
engines.  Adapters are stored under the folder :origin:`aperion/engines`.
...
It is required to tell Aperion Search the type of results the engine provides. The
...
.. table:: Common options in the engine setup (``settings.yml``)
   ...
   engine                  string      name of aperion-engine (file name without ``.py``)
...
aperion will not start with that engine (global names with a leading underline can
be ``None``).
...
.. table:: If the ``engine_type`` is :py:obj:`online
           <aperion.search.processors.online.OnlineProcessor.get_params>`
   :width: 100%
   ...
   aperion_locale         str            aperion's locale selected by user.  Specific language code like
...
.. table:: If the ``engine_type`` is :py:obj:`online_dictionary
           <aperion.search.processors.online_dictionary.OnlineDictionaryProcessor.get_params>`,
...
.. table:: If the ``engine_type`` is :py:obj:`online_currency
           <aperion.search.processors.online_currency.OnlineCurrencyProcessor.get_params>`,
           in addition to the :ref:`online <engine request online>` arguments:
   :width: 100%
...
.. table:: If the ``engine_type`` is :py:obj:`online_url_search
           <aperion.search.processors.online_url_search.OnlineUrlSearchProcessor.get_params>`,
           in addition to the :ref:`online <engine request online>` arguments:
   :width: 100%
...
<aperion.engines.demo_online.request>` always returns the ``params`` variable, the
