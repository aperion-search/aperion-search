...
.. sidebar:: offline engines
   ...
   - :ref:`sql engines`
   - :ref:`nosql engines`
   - :py:obj:`aperion.search.processors.offline`

To extend the functionality of Aperion Search, offline engines are going to be
introduced.  An offline engine is an engine which does not need Internet
connection to perform a search and does not use HTTP to communicate.

Offline engines can be configured, by adding those to the `engines` list of
:origin:`settings.yml <aperion/settings.yml>`.  An example skeleton for offline
engines can be found in :ref:`demo offline engine` (:origin:`demo_offline.py
<aperion/engines/demo_offline.py>`).


Programming Interface
=====================

:py:func:`init(engine_settings=None) <aperion.engines.demo_offline.init>`
...
:py:func:`search(query, params) <aperion.engines.demo_offline.search>`
...
If an offline engine depends on an external tool, Aperion Search does not install it by
...
Environment* of your aperion instance you need to switch into the environment
(:ref:`aperion-src`) first, for this you can use :ref:`aperion.sh`::
...
query contains a valid token, then aperion performs the requested private
