...
.. sidebar:: KISS_ and readability_
   ...
   Instead of defining more and more roles, we at aperion encourage our
   contributors to follow principles like KISS_ and readability_.

We at aperion are using reStructuredText (aka reST_) markup for all kind of
documentation.  With the builders from the Sphinx_ project a HTML output is
generated and deployed at docs.aperion.org_.  For build prerequisites read
:ref:`docs build`.

.. _docs.aperion.org: https://docs.aperion.org/

The source files of aperion's documentation are located at :origin:`docs`.
...
Admin's POV: :origin:`docs/admin`
  ...
  not know all the pros and cons of a aperion setup.
...
.. code:: sh
   ...
   ...
   $ python -m sphinx.ext.intersphinx https://docs.aperion.org/objects.inv
   ...
...
aperion's sphinx setup includes: :ref:`linuxdoc:kfigure`.  Scalable here means;
...
install` (with aperion modules installed).  We use this e.g. to build chapter:
...
The context for the template is selected in the line ``.. jinja:: aperion``.  In
sphinx's build configuration (:origin:`docs/conf.py`) the ``aperion`` context
...
.. code:: py
   ...
   jinja_contexts = {
      'aperion': {
         'engines': aperion.engines.engines,
         'plugins': aperion.plugins.plugins
      },
   }
