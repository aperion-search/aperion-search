...
.. jinja:: aperion
   ...
   Aperion Search supports {{engines | length}} search engines of which
...
.. jinja:: aperion
   ...
   .. flat-table::
      ...
      * - `{{mod.name}} <{{mod.about and mod.about.website}}>`_
          ...
          {%- endif %}
        - ``!{{mod.shortcut}}``
        - {%- if 'aperion.engines.' + mod.__name__ in documented_modules %}
          :py:mod:`~aperion.engines.{{mod.__name__}}`
          {%- else %}
          :origin:`{{mod.__name__}} <aperion/engines/{{mod.__name__}}.py>`
          {%- endif %}
