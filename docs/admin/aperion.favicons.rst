...
Activating the favicons in aperion is very easy, but this **generates a
...
To activate favicons in aperion's result list, set a default
...
By default and without any extensions, aperion serves these resolvers:
...
TOML_ configuration is created in the file ``/etc/aperion/favicons.toml``.
...
.. code:: toml
   ...
   db_url = "/var/cache/aperion/faviconcache.db"  # default: "/tmp/faviconcache.db"
...
:py:obj:`cache.db_url <.FaviconCacheConfig.db_url>`:
  ...
  cache of applications, so a suitable storage location of aperion's caches is
  folder ``/var/cache/aperion``.

  In a standard installation (compare :ref:`create aperion user`), the folder
  must be created and the user under which the aperion process is running must
  be given write permission to this folder.

  .. code:: bash

     $ sudo mkdir /var/cache/aperion
     $ sudo chown root:aperion /var/cache/aperion/
     $ sudo chmod g+w /var/cache/aperion/
...
aperion hosters can change other parameters of the cache as required:
...
.. code:: bash
   ...
   $ python -m aperion.favicons cache maintenance

The following command can be used to display the state of the cache:

.. code:: bash

   $ python -m aperion.favicons cache state
...
already set sensibly with settings from the :ref:`settings.yml <aperion
...
:py:obj:`max_age <.FaviconProxyConfig.max_age>`:
  ...
  As a rule, in the favicons infrastructure of aperion's this setting only
...
infrastructure of aperion generates this ``resolver_map`` automatically
depending on the ``settings.yml``.  aperion would automatically generate the
...
.. code:: toml
   ...
   "duckduckgo" = "aperion.favicons.resolvers.duckduckgo"
...
.. code:: toml
   ...
   "duckduckgo" = "aperion.favicons.resolvers.duckduckgo"
   "allesedv" = "aperion.favicons.resolvers.allesedv"
   # "google" = "aperion.favicons.resolvers.google"
   # "yandex" = "aperion.favicons.resolvers.yandex"
...
In the following we list the resolvers available in the core of aperion, but via
the FQN_ it is also possible to implement your own resolvers and integrate them
into the *proxy*:

- :py:obj:`aperion.favicons.resolvers.duckduckgo`
- :py:obj:`aperion.favicons.resolvers.allesedv`
- :py:obj:`aperion.favicons.resolvers.google`
- :py:obj:`aperion.favicons.resolvers.yandex`
