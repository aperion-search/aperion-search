...
find a description to test your valkey connection in aperion.

``url`` : ``$aperion_VALKEY_URL``
  ...
  When using sockets, don't forget to check the access rights on the socket::
    ...
    ls -la /usr/local/aperion-valkey/run/valkey.sock
    srwxrwx--- 1 aperion-valkey aperion-valkey ... /usr/local/aperion-valkey/run/valkey.sock

  In this example read/write access is given to the *aperion-valkey* group.  To
  get access rights to valkey instance (the socket), your aperion (or even your
  developer) account needs to be added to the *aperion-valkey* group.
...
.. code:: sh
   ...
   $ ./utils/aperion.sh install valkey
   # restart your aperion instance
